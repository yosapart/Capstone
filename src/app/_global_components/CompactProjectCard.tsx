...
export function CompactProjectCard({ project, onEdit, onDelete, onView }: ProjectCardProps) {
  const tag = project.flows_stats && project.flows_stats.length > 0 ? project.flows_stats[0].name : "Draft";

  return (
    <div className="group bg-white rounded-xl border border-gray-200 p-4 flex flex-col justify-between hover:shadow-lg hover:border-[#5d88bd]/30 hover:bg-slate-50/50 transition-all duration-200 relative overflow-hidden h-[110px]">
      ...
      <div className="flex justify-between items-start">
        <div className="flex flex-col gap-1 overflow-hidden pr-2">
          <Link
            ...
            className="font-bold text-gray-900 text-[16px] truncate hover:text-[#5d88bd] transition-colors"
          >
            {project.name}
          </Link>
      ...
      <div className="flex justify-between items-end mt-auto pt-2">
        ...
        <div className="flex items-center gap-1.5 shrink-0 invisible opacity-0 translate-x-2 group-hover:translate-x-0 group-hover:visible group-hover:opacity-100 transition-all duration-200">
            <button
